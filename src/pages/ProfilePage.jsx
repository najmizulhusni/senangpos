...
import { User, Phone, Save, Mail, Calendar, Shield, Globe, HelpCircle, Send, MessageCircle, ChevronDown, Bug, Lightbulb, CreditCard, UserCog, MoreHorizontal } from 'lucide-react';
...
export default function ProfilePage({ initialTab = 'profile' }) {
  ...
  const handleSendSupport = () => {
    ...
    const text = `*${supportForm.subject}*\n\n${supportForm.message}\n\n---\nFrom: ${form.full_name || 'User'}\nShop: ${form.shop_name || '-'}\nEmail: ${user?.email || '-'}`;
      ...
      <div className="max-w-5xl mx-auto p-4 lg:p-6">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-center bg-gray-100 rounded-xl p-1">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    language === 'en' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  🇬🇧 English
                </button>
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    language === 'ms' ? 'bg-white text-emerald-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  🇲🇾 Melayu
                </button>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                    activeTab === 'profile' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2 ${
                    activeTab === 'support' ? 'bg-emerald-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'
                  }`}
                >
            ...
            {activeTab === 'profile' && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 focus:outline-none transition-all"
                    />
                  </div>
                  <div>
                ...
                <button
                  onClick={handleSave}
                  disabled={loading}
                  className="mt-5 bg-emerald-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-emerald-700 transition-all flex items-center gap-2 disabled:opacity-50"
                >
                  <Save size={18} />
            ...
            {/* Support Form with Custom Dropdown */}
            {activeTab === 'support' && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800">{t('contactSupport')}</h3>
                ...
                <div className="space-y-4 mt-5">
                  ...
                  <div>
                    ...
                    <div className="relative" ref={dropdownRef}>
                      <button
                        ...
                        className={`w-full flex items-center justify-between border rounded-xl px-4 py-3 text-sm transition-all ${
                          dropdownOpen ? 'border-emerald-500 ring-2 ring-emerald-100' : 'border-gray-200 hover:border-gray-300'
                        }`}
                      >
                        {selectedSubject ? (
                        ...
                        <div className="absolute z-10 w-full mt-2 bg-white border border-gray-200 rounded-xl shadow-lg overflow-hidden">
                          {SUPPORT_SUBJECTS.map((subject) => (
                            <button
                              ...
                              type="button"
                              onClick={() => {
                                setSupportForm({ ...supportForm, subject: subject.value });
                                setDropdownOpen(false);
                              }}
                              className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors ${
                                supportForm.subject === subject.value ? 'bg-emerald-50' : ''
                              }`}
                            >
                  ...
                  <button
                    ...
                    className="w-full bg-green-600 text-white py-3.5 rounded-xl font-semibold hover:bg-green-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Send size={18} />
                    {t('sendToWhatsApp')}
                  </button>
